// API基础URL
// const BASE_URL = 'https://ekrlspkbrykg.sealosbja.site/api';
const BASE_URL = 'https://xubapwweknjk.sealosbja.site/api';
// 获取存储的token
const getToken = () => {
...
const request = async (url, method = 'GET', data = null) => {
    ...
    console.log(`API请求: ${method} ${requestUrl}`, data ? { data } : '');
            ...
            success: (res) => {
                ...
                if (res.statusCode === 200 || res.statusCode === 201) {
                    ...
                    if (result && typeof result === 'object' && 'code' in result) {
                        ...
                        // 如果响应数据没有 code 字段，直接返回数据（如 /chat/sessions 接口）
                        resolve(result);
                    }
                } else {
                    const error = new Error(result?.message || '请求失败');
...
export const chatApi = {
    ...
    updateSessionTitle: (sessionId, title) => {
        return request(`/chat/sessions/${sessionId}`, 'PUT', { title });
    },
